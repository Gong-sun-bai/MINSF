...
def ori(model, name, logger, optimizer_type='SGD',batch_size = 700,learning_rate = 0.001,num_epochs = 100):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'./images_optimizer/{name}_loss_{optimizer_type}.png')
    plt.close()
    ...
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.savefig(f'./images_optimizer/{name}_accuracy_{optimizer_type}.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    ori(model,'CNN',logger,'Adam')
    ori(model,'CNN',logger,'RMSprop')
