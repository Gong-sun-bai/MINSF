...
def ori(model, name, logger, optimizer_type='SGD',batch_size = 700,learning_rate = 0.001,num_epochs = 100):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'./images_lr/{name}_loss_{optimizer_type}_{learning_rate}.png')
    plt.close()
    ...
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.savefig(f'./images_lr/{name}_accuracy_{optimizer_type}_{learning_rate}.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    ori(model,'CNN',logger,learning_rate=0.1)
    ori(model,'CNN',logger,learning_rate=0.001)
    ori(model,'CNN',logger,learning_rate=0.00001)